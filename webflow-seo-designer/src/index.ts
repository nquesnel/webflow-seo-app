...
async function analyzeKeywordPlacement(page: any, keyword: string): Promise<SEOIssue[]> {
  ...
  try {
    ...
    const imageKeywordAnalysis = await checkKeywordInImages(rootElement, lowerKeyword);
...
async function checkKeywordInImages(element: any, keyword: string): Promise<SEOIssue | null> {
  let totalImages = 0;
