    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (page) {
            try {
                ...
                if (rootElement) {
                    const h1Count = yield countHeadingsByLevel(rootElement, 1);
                    if (h1Count === 0) {
                    ...
                    const imageIssues = yield checkImagesForAltText(rootElement);
        ...
        displayRecommendations(issues, (contentCache === null || contentCache === void 0 ? void 0 : contentCache.headings) || []);
    });
}
function setupEventListeners() {
    var _a, _b;
    // Refresh button
    (_a = document.getElementById('refresh-btn')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
    ...
    descInput === null || descInput === void 0 ? void 0 : descInput.addEventListener('input', () => {
        updateCharCount('desc', descInput.value);
        enableUpdateButton();
        // Re-analyze in real-time
        analyzePageRealTime();
    });
    keywordInput === null || keywordInput === void 0 ? void 0 : keywordInput.addEventListener('input', () => {
    ...
    // Update meta button
    (_b = document.getElementById('update-meta')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', updateMetaTags);
}
function analyzePage() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const issues = yield analyzePageContent(page);
...
function analyzePageContent(page) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (keyword) {
            const keywordPlacement = yield analyzeKeywordPlacement(page, keyword);
        ...
        try {
            const rootElement = yield webflow.getRootElement();
            if (rootElement) {
                const linkAnalysis = yield analyzeLinkStructure(rootElement);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (slug) {
                ...
                const keywordInSlug = keywordWords.every(word => slugWords.some(slugWord => slugWord.includes(word)));
                if (keywordInSlug) {
            ...
            const rootElement = yield webflow.getRootElement();
            if (rootElement && (contentCache === null || contentCache === void 0 ? void 0 : contentCache.headings)) {
                const h1Headings = contentCache.headings.filter(h => h.level === 1);
                const h1WithKeyword = h1Headings.some(h => h.text.toLowerCase().includes(lowerKeyword));
                ...
                const subheadingWithKeyword = subheadings.some(h => h.text.toLowerCase().includes(lowerKeyword));
            ...
            if (contentCache === null || contentCache === void 0 ? void 0 : contentCache.text) {
...
function collectAllLinks(element) {
    return __awaiter(this, void 0, void 0, function* () {
        const links = [];
            ...
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    if (el.getTag && typeof el.getTag === 'function') {
                        ...
                        if (tag === 'a') {
                            const href = yield el.getAttribute('href');
                            const rel = yield el.getAttribute('rel');
                            let linkText = '';
                            // Extract link text
                            if (el.getChildren && typeof el.getChildren === 'function') {
                                ...
                                for (const child of children) {
                                    if (child.type === 'String') {
                                        const text = yield child.getText();
                                        if (text)
                                            linkText += text + ' ';
                                    }
                                }
                            ...
                            if (href) {
                                links.push({
                                    href: href,
                                    text: linkText.trim(),
                                    nofollow: (rel === null || rel === void 0 ? void 0 : rel.includes('nofollow')) || false
                                });
                            }
                        }
                    }
...
function countHeadingsByLevel(element, level) {
    return __awaiter(this, void 0, void 0, function* () {
        let count = 0;
        try {
            ...
            if (element.type === 'Heading') {
                ...
                const headingLevel = yield element.getHeadingLevel();
            ...
            if (element.getChildren && typeof element.getChildren === 'function') {
                ...
                if (children && Array.isArray(children)) {
                    for (const child of children) {
                        count += yield countHeadingsByLevel(child, level);
                    }
                }
            }
        }
...
function calculateSEOScore(issues, contentAnalysis) {
    ...
    const keywordInput = document.getElementById('focus-keyword');
    const hasKeyword = (keywordInput === null || keywordInput === void 0 ? void 0 : keywordInput.value.trim().length) > 0;
    ...
    if (titleIssues.some(i => i.type === 'success'))
        metaScore += 50;
    if (descIssues.some(i => i.type === 'success'))
        metaScore += 50;
    // Heading score (15% weight)
    const h1Issues = issues.filter(i => i.message.includes('H1'));
    if (h1Issues.some(i => i.type === 'success'))
        headingScore = 100;
    else if (h1Issues.some(i => i.type === 'warning'))
        headingScore = 50;
    // Image score (10% weight)
    const imageIssues = issues.filter(i => i.message.includes('image'));
    if (imageIssues.some(i => i.type === 'success'))
        imageScore = 100;
    else if (imageIssues.some(i => i.type === 'warning'))
        imageScore = 50;
    ...
    if (contentAnalysis) {
        ...
        // Word count scoring
        let wordScore = 0;
        if (wordCount >= SEO_CONFIG.content.optimalWords)
            wordScore = 100;
            ...
            wordScore = (wordCount / SEO_CONFIG.content.minWords) * 60;
        // Readability scoring
        let readScore = 0;
    ...
    const finalScore = ((metaScore * 0.20) +
        (headingScore * 0.15) +
        (imageScore * 0.10) +
        (contentScore * 0.35) +
        (keywordScore * 0.20));
...
function displaySEOScore(score) {
    ...
    if (score.total === -1) {
        ...
        if (scoreLabel)
            scoreLabel.textContent = 'Add Focus Keyword';
        if (scoreCircle) {
    ...
    if (scoreValue)
        scoreValue.textContent = score.total.toString();
    ...
    if (scoreCircle) {
        ...
        if (score.total >= 80) {
            ...
            if (scoreLabel)
                scoreLabel.textContent = 'Good SEO';
        }
            ...
            if (scoreLabel)
                scoreLabel.textContent = 'Poor SEO';
        }
    }
}
...
function displayRecommendations(issues, headings) {
    var _a;
    ...
    // Get current metrics
    const wordCount = parseInt(((_a = document.getElementById('word-count')) === null || _a === void 0 ? void 0 : _a.textContent) || '0');
    const keywordInput = document.getElementById('focus-keyword');
    const hasKeyword = (keywordInput === null || keywordInput === void 0 ? void 0 : keywordInput.value.trim().length) > 0;
    ...
    if (!hasKeyword) {
        recommendations.push({
            ...
            action: () => {
                keywordInput === null || keywordInput === void 0 ? void 0 : keywordInput.focus();
            }
        });
    }
    ...
    issues.filter(i => i.type === 'error').forEach(issue => {
        if (issue.message.includes('No H1')) {
            ...
            recommendations.push({
                ...
                action: () => {
                    const titleInput = document.getElementById('page-title');
                    titleInput === null || titleInput === void 0 ? void 0 : titleInput.focus();
                }
            });
        }
            ...
            recommendations.push({
                ...
                action: () => {
                    const descInput = document.getElementById('page-description');
                    descInput === null || descInput === void 0 ? void 0 : descInput.focus();
                }
            });
        }
    });
    ...
    if (hasKeyword && (contentCache === null || contentCache === void 0 ? void 0 : contentCache.text)) {
...
function updateCharCount(field, value) {
...
function enableUpdateButton() {
    const button = document.getElementById('update-meta');
    if (button) {
...
function updateMetaTags() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const descInput = document.getElementById('page-description');
        const button = document.getElementById('update-meta');
        if (!titleInput || !descInput || !button)
            return;
        ...
        try {
            ...
            yield page.setTitle(titleInput.value || null);
            yield page.setDescription(descInput.value || null);
...
function analyzeContentQuality() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const textContent = yield extractTextContent(rootElement);
            if (textContent.length === 0) {
                ...
                const headingStructure = yield analyzeHeadingStructure(rootElement);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (element.type === 'String') {
                try {
                    const textContent = yield element.getText();
            ...
            if (element.getTag && typeof element.getTag === 'function') {
                try {
                    const tag = yield element.getTag();
            ...
            if (element.textContent === true && element.children) {
                try {
                    ...
                    for (const child of children) {
                        if (child.type === 'String') {
                            const childText = yield child.getText();
                            if (childText) {
                                text += childText + ' ';
                            }
                        }
                ...
                try {
                    ...
                    if (children && Array.isArray(children)) {
                        for (const child of children) {
                            ...
                            if (child.type !== 'String') {
                                const childText = yield extractTextContent(child);
                                if (childText) {
                                    text += childText + ' ';
                                }
                            }
...
function analyzeContentQualityMetrics(text) {
...
function analyzeHeadingStructure(element_1) {
    return __awaiter(this, arguments, void 0, function* (element, headings = []) {
        try {
            // Check if this element is a Heading type
            if (element.type === 'Heading') {
                try {
                    // Get the heading level (1-6)
                    const level = yield element.getHeadingLevel();
                    let text = '';
                ...
                try {
                    ...
                    if (children && Array.isArray(children)) {
                        for (const child of children) {
                            ...
                            yield analyzeHeadingStructure(child, headings);
                        }
                    }
                }
                catch (e) {
...
function displayContentMetrics(wordCount, readingTime, readabilityScore) {
    // Word count - Update the entire metric-value-wrap div
    const wordCountEl = document.getElementById('word-count');
    const wordTargetEl = document.getElementById('word-target');
    const wordCountWrap = wordCountEl === null || wordCountEl === void 0 ? void 0 : wordCountEl.parentElement;
    ...
    const readabilityEl = document.getElementById('readability-score');
    const readabilityDescEl = document.getElementById('readability-desc');
    const readabilityWrap = readabilityEl === null || readabilityEl === void 0 ? void 0 : readabilityEl.parentElement;
